...
- stagger labels with a **vertical-first placement grid**: levels are sorted by price and near-adjacent ones form an overlap group; slots fill vertically first (alternating above/below, up to `每侧纵向行数` rows per side at `相邻间距阈值` pitch) and only once both sides are full does a new column open `横向列宽` bars further right — so pairs, trios and larger clusters of coincident levels stay readable without text overlap.
